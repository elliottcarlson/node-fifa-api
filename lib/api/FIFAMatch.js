...
var FIFAMatch = function () {
    function FIFAMatch(match) {
        ...
        match.HomeTeam.Players.forEach(function (player) {
            player.ShortName.forEach(function (info) {
                _this.players[player.IdPlayer] = info.Description;
            });
        });
    ...
    _createClass(FIFAMatch, [{
        ...
        key: 'getPlayerOrTeam',
        value: function getPlayerOrTeam(player, team) {
            return this.getPlayerName(player) + ' (' + this.getTeamName + ')' || this.getTeamName(team);
        }
    }, {
        key: 'getEvents',
